...
// .to(headerH2.words, {autoAlpha: 1, y: 0, rotationX:0, stagger:0.5, ease: "power2.out"})
// .to(headerH2.chars, {autoAlpha: 1, y: 0, rotationX:0, stagger:0.025, ease:"power2.out"})
// .to(".header .first-view .text ul li", {autoAlpha: 1, y:0, stagger: 0.06})
// .from(".header .first-view .text .sentence", {autoAlpha: 0, y: 100})
...
const featureTitlelAnimation = gsap.timeline({
  defaults: {
    autoAlpha: 0, 
    filter: "blur(30px)",
    x: -100,
...
ctaElements.forEach((target) => {
  ...
  ctaAnimation
  .from(inner , {filter: "blur(60px)", duration: 1.9})
...
.to(footer, {
  backgroundImage:
  "linear-gradient(90deg, rgba(0,0,0,1) 0%, rgba(0,0,0, 1) 100%", duration: 1.25,
})
